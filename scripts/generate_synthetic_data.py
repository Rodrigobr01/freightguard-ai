...
def generate_invoices(shipments: pd.DataFrame, n: int = 2200) -> pd.DataFrame:
    ...
    sampled["data_fatura"] = pd.to_datetime(sampled["data_prevista"]) + pd.to_timedelta(
        rng.integers(0, 20, size=n), unit="D"
    )
    ...
    sampled.loc[date_issue_idx, "data_fatura"] = pd.to_datetime(
        sampled.loc[date_issue_idx, "data_coleta"]
