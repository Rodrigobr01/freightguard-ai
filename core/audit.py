...
def run_audit(
    ...
    merged = invoices.merge(
        shipments, on="shipment_id", how="left", suffixes=("_invoice", "_shipment")
    )
    merged["duplicate_invoice"] = merged.duplicated(subset=["shipment_id"], keep=False)

    breakdowns = merged.apply(
        lambda r: calculate_expected_freight(r.to_dict(), rates), axis=1, result_type="expand"
    )
    audited = pd.concat([merged, breakdowns], axis=1)

    extracted = audited.apply(
        lambda r: extract_invoice_text(str(r["itens_taxa"]), str(r["observacao"])), axis=1
    )
    ...
    fallback_used = (
        bool(audited["fallback_used"].any()) if "fallback_used" in audited.columns else True
    )
    append_audit_trail(
