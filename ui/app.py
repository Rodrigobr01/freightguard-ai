...
if "audit_df" in st.session_state:
    ...
    trp = col2.selectbox(
        "Transportadora", ["todas"] + sorted(df["transportadora"].unique().tolist())
    )
    ...
    st.download_button(
        "Export CSV", filt.to_csv(index=False).encode("utf-8"), "auditoria_filtrada.csv", "text/csv"
    )

    if not filt.empty:
        idx = st.number_input(
            "Linha para contestação", min_value=0, max_value=len(filt) - 1, value=0
        )
